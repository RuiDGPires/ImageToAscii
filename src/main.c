...
char *doStuff(FIBITMAP *bm){
    ...
    *bm = *FreeImage_Rescale(bm, out_width, out_height, FILTER_BOX);

    char *buffer;
    unsigned buffer_size = sizeof(char)*(out_width + 1)*out_height;
    unsigned write_index = buffer_size - 1;
...
int main(int argc, char *argv[]){
    ...
    FIBITMAP *bitmap = FreeImage_Load(FIF_PNG, "github.png", BMP_DEFAULT);
    if (bitmap) {
